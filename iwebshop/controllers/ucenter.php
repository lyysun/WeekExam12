...
class Ucenter extends IController implements userAuthorization
{
    ...
    function invoice_del()
    {
     ...
     function do_email(){
        $addr=IReq::get("addr");
        $user_id=ICookie::get("user_id");
        $mail=new SendMail();
        $title="来加入我们吧";
        $url="http://www.shop.com/simple/reg/id/".base64_encode($user_id);
        $content="欢迎你注册成为我们的会员，我们的注册网址是".$url;
        ...
        //      if($mail->send($addr,$title,$content)){
        //                $data=array("code"=>1,"msg"=>"发送成功");
        //         }else{
        //              $data=array("code"=>2,"msg"=>"发送失败");
        //         }
        // }
